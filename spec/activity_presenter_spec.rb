...
describe TimeBudget::Presenters::ActivityPresenter do
  ...
  context "when displaying the duration" do
    it "displays the hours of the duration" do
      TimeBudget::Presenters::ActivityPresenter.new(stub("activity", duration: (60*60*12)+(60*30))).hours.should == "12"
    end

    it "pads the hours with one zero if the value is a single digit" do
      TimeBudget::Presenters::ActivityPresenter.new(stub("activity", duration: (60*60)+(60*30))).hours.should == "01"
    end

    it "displays the minutes of the duration" do
      TimeBudget::Presenters::ActivityPresenter.new(stub("activity", duration: (60*60*12)+(60*30))).minutes.should == "30"
    end

    it "pads the minutes with one zero if the value is a single digit" do
      TimeBudget::Presenters::ActivityPresenter.new(stub("activity", duration: (60*60*12)+(60*2))).minutes.should == "02"
    end
  end
end
